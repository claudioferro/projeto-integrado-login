...
public class Principal extends JFrame {
	...
	private void criarMenus() {
		...
		itemSalvar.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent evt) {
				JOptionPane.showMessageDialog(null,
						"Contatos salvos com sucesso.", "Salvar",
		 ...
		 * 
		 * public void actionPerformed(ActionEvent evt) { for (Conversa conversa
		 * : telaConversas) { try { conversa.setIcon(true); } catch
		 * (PropertyVetoException e) { e.printStackTrace(); } } try {
		 * listaAmigos.setIcon(true); listaNegra.setIcon(true); } catch
		...
		JButton adicionar = new JButton(
				new ImageIcon("resources/adicionar.png"));
	...
	public void fechaConversa(InternalFrameEvent internalFrame) {
		/*
		 * for (int i = 0; i < telaConversas.size(); i++) { Conversa temp =
		 * telaConversas.get(i); if(internalFrame.getInternalFrame().hashCode()
		 * == temp.hashCode()){ for(int j=0; j <
		 * toolBarConversas.getComponents().length; j++){ Component
		 * componentes[] = toolBarConversas.getComponents(); JButton botao =
		 * (JButton) componentes[j]; if (conversas.get(i).getCodigoJanela() ==
		 * botao.hashCode()){ toolBarConversas.remove(j); } }
		 * telaConversas.remove(i); conversas.remove(i);
		 * toolBarConversas.repaint(); break; } } if (telaConversas.size() ==
		 * 0){ toolBarConversas.setVisible(false); }
		 */
	}

	/*
	 * public void novaConversa(Contato amigo){ boolean estaAberta = false;
	 * Conversa janelaConversa = procurarJanela(amigo); if(janelaConversa !=
	 * null){ estaAberta = true; }
	 * 
	 * if (estaAberta){ janelaConversa.toFront(); } else { Conversa novaConversa
	 * = new Conversa(this, amigo, 20*conversas.size(), 22*conversas.size());
	 * telaConversas.add(novaConversa); conversas.add(amigo);
	 * novaJanela(novaConversa);
	 * 
	 * ThreadClient threadClient = new ThreadClient(amigo, "");
	 * threadClient.start();
	 * 
	 * String nomeConversa = amigo.getNome(); if (amigo.getNome().length() >
	 * 10){ nomeConversa = amigo.getNome().substring(0, 8) + "..."; }
	 * 
	 * JButton botaoConversa = new JButton(nomeConversa, new
	 * ImageIcon("resources/conversa.png"));
	 * amigo.setCodigoJanela(botaoConversa.hashCode());
	 * botaoConversa.addActionListener(new ActionListener() { public void
	 * actionPerformed(ActionEvent evt) { for (int i = 0; i < conversas.size();
	 * i++) { Contato contato = conversas.get(i); if(contato.getCodigoJanela()
	 * == evt.getSource().hashCode()){ if (telaConversas.get(i).isIcon()) { try
	 * { telaConversas.get(i).setIcon(false);
	 * telaConversas.get(i).setSelected(true); } catch (PropertyVetoException e)
	 * { e.printStackTrace(); } } telaConversas.get(i).toFront(); try {
	 * telaConversas.get(i).setSelected(true); } catch (PropertyVetoException e)
